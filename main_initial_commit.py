...
# 获取 PyInstaller 的临时工作目录
def get_pyinstaller_temp_dir():
    if getattr(sys, 'frozen', False):
        # 当脚本被 PyInstaller 打包时，使用 sys._MEIPASS 获取临时工作目录
        return sys._MEIPASS
    else:
        # 如果脚本未被打包，则返回当前工作目录
        return os.path.dirname(os.path.abspath(__file__))

# 在指定目录中搜索 ChromeDriver
def find_chromedriver():
    ...
    for root, dirs, files in os.walk(current_dir):
        for file in files:
            ...
            elif file == "chromedriver":
                return os.path.join(root, file)
    return None

# 复制 ChromeDriver 到 PyInstaller 的临时工作目录
def copy_chromedriver_to_temp_dir():
    pyinstaller_temp_dir = get_pyinstaller_temp_dir()
    chromedriver_source = find_chromedriver()
    if chromedriver_source:
        chromedriver_dest = os.path.join(pyinstaller_temp_dir, "chromedriver.exe")
        if not os.path.exists(chromedriver_dest):
            shutil.copy(chromedriver_source, chromedriver_dest)

# 自动检测 ChromeDriver
def initialize_driver():
    copy_chromedriver_to_temp_dir()  # 在初始化 WebDriver 之前复制 ChromeDriver
    chromedriver_path = os.path.join(get_pyinstaller_temp_dir(), "chromedriver.exe")
